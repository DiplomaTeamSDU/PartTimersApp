...
from .models import Category, Company, CustomUser, Education, Experience, Freelancer, Job, JobApplication, Rating, Project, Skill, Submission
...
def post_job(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if job.timeline < timezone.now().date():
                messages.error(request, 'Job timeline cannot be in the past')
                return redirect('post_job')  # Redirect back to the job posting page
...
def edit_job(request, job_id):
    ...
    if job.status == 'pending' and request.method == 'POST' and 'delete' in request.POST:
        job.delete()
